...
class qdigest_summary {
    ...
    class builder {
      ...
      public:
        ...
        qdigest_summary build() {
            ...
            std::for_each(m_vals.begin(), m_vals.end(),
  ...
  private:
    ...
    // This uses a stateful predicate trick from
    // http://www.informit.com/articles/article.aspx?p=25142&seqNum=4
      ...
      private:
        struct find_state {
            ...
            find_state(double desired_rank_)
                : curr_rank(0), desired_rank(desired_rank_) {}
        };
    ...
    qdigest_summary(int k, int max)
        : m_k(k), m_n(0), m_root(node_data(0, max)) {}
    ...
    void insert(int v, int count = 1) {
        ...
        while (n->data.lb != n->data.ub) {
            ...
            if (v <= mid) {
                if (!n->left) {
                    n->left.reset(new binary_tree_node<node_data>(
                        node_data(n->data.lb, mid)));
                }
                n = n->left.get();
            } else {
                assert(mid + 1 <= n->data.ub);
                if (!n->right) {
                    n->right.reset(new binary_tree_node<node_data>(
                        node_data(mid + 1, n->data.ub)));
                }
                n = n->right.get();
            }
        }
    ...
    void compress() { compress_node(&m_root); }
    ...
    void compress_node(binary_tree_node<node_data> *n) {
        ...
        int subtree_count = n->data.count +
                            (n->left ? n->left->data.count : 0) +
                            (n->right ? n->right->data.count : 0);
...
class qdigest {
  ...
  public:
    explicit qdigest(int compression_factor) : m_builder(compression_factor) {}

    void insert(int v, int count = 1) { m_builder.insert(v, count); }

    int quantile(double phi) { return m_builder.build().quantile(phi); }
};
} // namespace stmpct
