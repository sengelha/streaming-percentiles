...
namespace stmpct {
    ...
    struct binary_tree_node
    {
        ...
        binary_tree_node(const T& _data)
            : data(_data) {}
        ...
        friend std::ostream& operator<<(std::ostream& os, const binary_tree_node& n) {
            os << n.data;
            if (n.left) {
                std::stringstream ss;
                ss << *n.left;
    ...
    const binary_tree_node<T>* postorder_find_if(const binary_tree_node<T>* n, UnaryPredicate p) {
        if (!n)
            return nullptr;
