...
TEST(ckms_uq, type_traits) {
    ...
    ASSERT_TRUE(
        std::is_move_constructible<ckms_uq<minimal_number_type>>::value);
    ASSERT_TRUE(std::is_nothrow_move_constructible<
                ckms_uq<minimal_number_type>>::value);
    ...
    ASSERT_TRUE(
        std::is_nothrow_move_assignable<ckms_uq<minimal_number_type>>::value);
}
...
16, 17}; ckms_uq c(0.1); for (int i = 0; i < ARRAYSIZE(seq); ++i)
        c.insert(seq[i]);
